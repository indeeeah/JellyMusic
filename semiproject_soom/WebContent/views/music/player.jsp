...
<body>
	...
	<div id="player_left">
		...
		<c:if test="${not empty scm }">
			<c:forEach items="${scm }" var="v" varStatus="s">
				<div id="player_info">
					<div id="pt1">
						<a href="#" class="album" id="albumTi">${v.al_name} - </a><a
							href="#" class="arti" id="artiName">${v.art_name}</a>
					</div>
 ...
 --%>
					...
					<c:forEach items="${mplist }" var="v" varStatus="s">
						<div class="mulist" id="${v.mu_no }fordelete">
							...
							<a
								...
								class="mulist_con"> <img src="/image/${v.f_name }.jpg"
								class="mulist_album"></img>
								<p class="mulist_title">${v.mu_name }</p>
								<p class="mulist_artist">${v.art_name }</p>
							</a> <input type="hidden" class="nowmuno" name="mu_no1" id="muuno"
								value=${v.mu_no }> <input type="hidden"
								class="nowmuname" name="mu_name" value=${v.mu_name }> <input
								type="hidden" class="nowartname" name="art_name"
								value=${v.art_name }> <input type="hidden"
								class="nowalname" name="al_name" value=${v.al_name }> <input
								type="hidden" class="nowmuly" name="mu_ly" value=${v.mu_ly }>
						</div>
					</c:forEach>
				</c:if>
			</div>
	...
	<c:if test="${not empty scm }">
		<c:forEach items="${scm }" var="v" varStatus="s">
			<img
				src="${pageContext.request.contextPath}/image/${v.albumfile}.jpg"
				id="player_middle"></img>
		</c:forEach>
	</c:if>
	<div id="player_right">
		<img src="${pageContext.request.contextPath}/image/001-character.svg"
			id="userpro"></img>
		<div id="heart_con">
			<c:if test="${not empty heart }">
				<c:if test="${heart eq 0}">
					<input type="radio" name="heart_check" id="towhiteheart_chk"
						...
						value="heartup" class="heartchkbox" checked style="display: none;">
